...
public class UserService {
    ...
    public boolean isAuthorizedForUsers(String token){
        Algorithm algorithm = Algorithm.HMAC256("secret");
    ...
    public boolean isAuthorizedForCC(String token){
        Algorithm algorithm = Algorithm.HMAC256("secret");
        JWTVerifier verifier = JWT.require(algorithm).build();
        DecodedJWT jwt = verifier.verify(token);
        ...
        if(user == null)
            return false;
        if(tip.equalsIgnoreCase("content creator") && user.getActive() == false){
            return false;
        }
