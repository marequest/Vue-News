...
export default {
  ...
  mounted() {
    this.form.ime = this.userProp.ime
    this.form.prezime = this.userProp.prezime
    this.form.username = this.userProp.username
    this.form.tip = this.userProp.tip
    this.form.active = this.userProp.isActive
  },
  methods: {
    async onSubmit(event) {
      event.preventDefault()

      this.$axios.put('http://localhost:8081/api/users/' + this.form.username, {
        ime: this.form.ime,
        ...
        active: this.form.active
          ...
          .then(response => {
            ...
            console.log(error)
            alert("Greska u azuriranju korisnika!")
          })
    },
    onReset(event) {
      event.preventDefault()
      ...
      this.form.username = this.userProp.username
      this.form.tip = this.userProp.tip
      this.form.active = this.userProp.active
      this.form.password = this.userProp.password
