...
export default {
  ...
  name: "KategorijaKomp",
  mounted() {
    this.$axios.get('http://localhost:8081/api/categories/' + this.$route.params.id)
        .then(response => {
          this.home = response.data.naziv
        }).catch(error => {
          console.log(error)
    ...
    this.$axios.get('http://localhost:8081/api/news')
        .then(response => {
          for (const data of response.data) {
            if(data.kategorijaVesti == this.kategorija){
              this.vesti.push({ id: data.id, naslov: data.naslov, tekst: data.tekst, kategorija: data.kategorijaVesti, datum: data.vremeKreiranja})
            }
          }
  ...
  data() {
    return {
      kategorija: this.$route.params.id,
      home: "",
      totalRows: 1,
      currentPage: 1,
      perPage: 10,
